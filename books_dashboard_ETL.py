...
def transform_data():
    ...
    df['Price'] = df['Price'].apply(lambda x: ''.join(list(x.split(","))))
    df['Price'] = df['Price'].apply(lambda x: round(float(x) / 20.61, 2))
    df = df[['Title', 'Author', 'Main Genre', 'Sub Genre', 'Type', 'Rating', 'No. of People rated', 'Price']]
    df.to_pickle(r'./data/Books_df.pkl')

def save_to_excel():
    df = pd.read_pickle(r'./data/Books_df.pkl')
    df.to_excel(r'./data/Books_df.xlsx', index=False)
...
with DAG(
    ...
    description='Amazon Books Dashboars ETL',
